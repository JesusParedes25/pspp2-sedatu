...
import { ChevronDown, Plus, AlertTriangle, User, Shield, Trash2, ListOrdered, CalendarClock, Flag, BarChart3 } from 'lucide-react';
...
const CRITERIOS_ORDEN = [
  { clave: 'orden',   icono: ListOrdered,   etiqueta: 'Orden' },
...
export default function EtapaCard({ etapa, soloLectura = false, onAccionCreada, onEtapaActualizada }) {
  const [expandida, setExpandida] = useState(false);
  const [mostrarRiesgos, setMostrarRiesgos] = useState(false);
  const [riesgos, setRiesgos] = useState([]);
  const [ordenActivo, setOrdenActivo] = useState('orden');
  // ID de la acción actualmente expandida — solo una a la vez
  const [accionExpandidaId, setAccionExpandidaId] = useState(null);
  const { acciones, cargando, recargar, recargarSilencioso, actualizarPorcentaje } = useAcciones(
                  ...
                  return (
                    <button key={c.clave} onClick={() => setOrdenActivo(c.clave)}
                      className={`flex items-center gap-1 px-2 py-1 text-[11px] rounded-lg transition-colors ${
                        activo
                          ? 'bg-guinda-50 text-guinda-600 font-medium'
              ...
              <button
                onClick={async () => {
                  ...
                  try {
                    ...
                    alert(err.response?.data?.mensaje || 'Error al eliminar etapa');
                  }
                }}
            ...
            {cargando ? (
              ...
              accionesOrdenadas.map(accion => (
                <AccionItem
                  ...
                  expandida={accionExpandidaId === accion.id}
                  onToggleExpandir={(id) => {
                    setAccionExpandidaId(prev => prev === id ? null : id);
                  }}
                  onPorcentajeCambio={actualizarPorcentaje}
